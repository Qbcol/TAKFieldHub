    ...
    public string Build(FieldTakProject project, IReadOnlyList<ContentItem> items)
    {
        ...
        try
        {
            ...
            var enrollmentFile = Directory.Exists(enrollmentRoot)
                ? Directory.EnumerateFiles(enrollmentRoot, "enroll.url", SearchOption.TopDirectoryOnly).FirstOrDefault()
                : null;
            ...
            var meshChannel = Directory.Exists(meshRoot)
                ? Directory.EnumerateFiles(meshRoot, "channel.url", SearchOption.TopDirectoryOnly).FirstOrDefault()
                : null;
            if (meshChannel != null)
            {
            ...
            var meshProfile = Directory.Exists(meshRoot)
                ? Directory.EnumerateFiles(meshRoot, "profile.json", SearchOption.TopDirectoryOnly).FirstOrDefault()
                : null;
            if (meshProfile != null)
            {
                try
            ...
            var preliminary = new FieldTakManifest
            {
                ...
                Components = new ComponentInfo { MissionPackage = missionCreated, Plugins = pluginCount, Files = Directory.EnumerateFiles(payload, "*", SearchOption.AllDirectories).Count(), Enrollment = enrollmentFile != null, Meshtastic = project.Meshtastic.Enabled || (Directory.Exists(Path.Combine(project.SourceDirectory, "meshtastic")) && Directory.EnumerateFiles(project.SourceDirectory + Path.DirectorySeparatorChar + "meshtastic", "*", SearchOption.AllDirectories).Any()) },
