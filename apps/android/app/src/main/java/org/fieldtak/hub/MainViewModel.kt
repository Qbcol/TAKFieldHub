  ...
  fun onExternalReturn(){
    viewModelScope.launch(Dispatchers.IO){
      ...
      when(s.stage){
        ...
        DeploymentStage.OTS_ENROLLMENT -> s=engine.transition(s,DeploymentStage.OTS_ENROLLMENT,StepResult.WARNING,text(R.string.vm_enrollment_returned))
        else -> Unit
      }
      update(session=s)
      ...
      if(_state.value.trusted && (canContinueInstall || atakJustInstalled || pluginStepDone || missionReturned || enrollmentReturned)){
        preparePhone()
      } else preflight()
    }
  }
