...
class ServerDiagnostics(private val context:Context) {
  ...
  suspend fun run(server:ServerInfo,meshtastic:MeshtasticProfile=MeshtasticProfile()):ServiceReport = coroutineScope {
    ...
    val gateway=DiagnosticCheck(
      ...
      if(!meshtastic.enabled || meshtastic.deviceRole=="field_node") CheckState.UNKNOWN else if(meshtastic.gateway.enabled && meshtastic.gateway.secretsExternal && meshtastic.gateway.transport.equals("mqtt_tls",true) && meshtastic.gateway.rootTopic.isNotBlank()) CheckState.READY else CheckState.PROBLEM,
      if(!meshtastic.enabled || meshtastic.deviceRole=="field_node") s(R.string.diag_meshtastic_not_configured) else s(R.string.diag_gateway_config_detail,meshtastic.gateway.type,meshtastic.gateway.transport,meshtastic.gateway.mqttPort,meshtastic.gateway.rootTopic)
    )
